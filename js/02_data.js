...
/*  
    ...
    JS 에서는 정수와 실수를 구분하지 않는다.
*/
var year = 2022;
var PI = 3.14;

console.log(typeof year, typeof PI);
console.log(0.1 + 0.2); //부동소수점 표현 때문에 생기는 오차. 정밀한 수학계산에서는 적합X.

/*
    2. 문자열 - 문자를 나타내는 데이터. ""(큰따옴표)나 ''(작은따옴표) 안에 문자를 작성.
        =>JS에선 ""와 '' 구분 X => 문자와 문자열 구분하지 X
*/
var name = '"Lim    Hyebin "';
console.log(name);
var num04 = "1";
console.log(typeof num04 + 2); // 문자열과 숫자를 더하면 문자열끼리 더한다. => 오류 발생x.

/*
     논리형(boolean)  : 참과 거짓으로 표현되는 자료형 => true(참, 1), false(거짓, 0)
         => 조건식과 함께 사용한다. => 비교 연산자의 결과 값이 논리형 데이터가 반환된다.
 */
...
/*
    ...
    순서(인덱스)로 데이터를 참조 => 순서는 0 부터 시작.. 즉 첫번째는 0, 두번째는 1...
    인텍스 값으로는 숫자를 사용. => 숫자로 평가되는 "표현식" 사용 가능.
*/
...
/*
    ...
    "객체.속성명(key)"의 형태로 참조하면 해당 키에 저장되어 있는 값(value)을 참조할 수 있다. 
    "객체[속성명]"의 형태로도 참조 할 수 있다.
