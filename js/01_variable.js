/* 
    1. 변수 (Variable)
    - 메모리 공간에 데이터를 저장(할당) 할 수 있는 공간을 마련하고 이름(변수명)을 붙여서 해당 공간(주소)에 접근(참조)
    - 쉽게 표현하면 데이터를 보관하는 상자와 같은 역활을 합니다.
 */

/*
    2. 변수 선언(Declaration)
    - 데이터를 저장할 공간을 확보하고 해당 공간에 접근할수 있는 변수 명을 붙인다
    - var, let, const(상수) 키위드 뒤에 변수명을 짓는다.
    - 변수 명을 짓는 몇가지 규칙이 있다.
